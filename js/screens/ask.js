...
import { findAnswer } from '../core/ask-match.js';
...
/* Eşleştirme çekirdeği ayrı modülde: denetçi de aynı kodu kullanıyor. */

function answerHtml(question) {
  const bulunan = findAnswer(question);
  if (!bulunan) return noAnswerHtml();          // kaynak yoksa cevap yok

  if (bulunan.tur === 'hazir') return structured(bulunan.kayit);

  const a = bulunan.kayit;
  return structured({
    short: a.summary,
